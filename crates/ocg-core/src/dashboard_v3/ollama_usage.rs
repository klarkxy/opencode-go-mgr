...
//! the 30-second manual throttle, and reuses the process-wide
//! `provider_usage_refresh` lock. Usage failures never write inference
...
use crate::ollama_usage::{
    self, ParseOutcome, manual_next_allowed_at, normalize_cookie_header, sanitize_error_text,
};
use crate::provider::OLLAMA_PROVIDER_ID;
...
/// per 30 seconds (success or failure).
...
fn load_ollama_account(state: &CoreState, id: &str) -> Result<crate::models::Account, V3ApiError> {
    ...
    if account.provider_id != OLLAMA_PROVIDER_ID {
        return Err(V3ApiError::invalid_request_at(
            state,
...
mod tests {
    ...
    fn throttle_uses_the_thirty_second_manual_window() {
        ...
        state.last_attempt_at = Some(now - chrono::Duration::seconds(31));
        assert!(refresh_throttle(&state, now).is_none());
    }
}
