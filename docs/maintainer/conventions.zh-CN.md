...
- 只描述当前行为和明确限制。已知缺口放入 `docs/user/limits.md` 或
  ...
  计费的推理或已安装桌面版可用。
- 用肯定句写当前事实。章节先写产品做什么、读者该怎么做。只有当该页的首次读者
  合理会假设相反情况时，才补充否定——例如静默丢弃、看起来存在的控件、看起来
  受支持的平台，或会泄漏的凭据。历史纠正、被否决的设计和实现争论放在
  `limits.md`、`known-debt.md` 或本页；其他章节用链接指向它们，而不是再写一遍
  “不是什么”。
