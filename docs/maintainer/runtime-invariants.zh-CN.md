...
- 模型协议能力硬编码在 `ocg_domain::protocol` 的 `MODEL_PROTOCOLS` 中（`ocg-core` 的 `kernel/protocol.rs` 与 `gateway/protocol.rs` 是 facade/host 转换）：`preferred` 与官方 Go 文档端点表一致，`supported` 是使用测试账号验证后检入的集合；之后的显式探测观察另行持久化。有效模型首选协议优先于客户端协议；首选被禁用时才从已启用协议中按供应商顺序回退。CPA 保持支持的客户端协议。请求路径使用已保存合约；协议探测会导致重复计费。`grok-4.6`、`grok-4.5` 与 `gpt-5.6-luna` 都仅支持 Responses，因此 Chat 入口必须转换。`MODEL_PROTOCOLS` 目前仍只服务于 OpenCode Go；Go/Zen 刷新得到的表中未知 ID 按供应商级默认协议（Chat Completions）物化为可用，刷新写入的默认关覆盖使其保持关闭，直到显式开启。整篇 JSON 转换内核在 `ocg-gateway` 中。
