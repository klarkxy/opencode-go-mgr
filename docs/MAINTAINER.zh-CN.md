...
应用教程由 `src/views/application-guides.ts` 的 15 个条目驱动；改动注册表时
...
- Docker 可用 `OCG_ADMIN_USERNAME` 与 `OCG_ADMIN_PASSWORD` 引导首个管理员；
  不提供时由首位注册者创建。
- **应用** 视图维护 15 个教程：Claude Code、Claude Desktop、Codex、Gemini
  CLI、Pi、Kimi Code CLI、OpenCode、OpenClaw、Hermes、Cherry Studio、VS Code
  Copilot Chat、Cline、Roo Code、Continue、Chatbox。Claude Desktop 的复制动作还会
  保存三个角色模型；其他教程只生成客户端配置，不修改 Gateway 设置。
...
- [ ] 验证带鉴权的 Claude Desktop 模型发现与 Messages 别名改写。通过面板 API
      ...
      会话时映射 API 返回 `401`。
- [ ] 打开 **应用** 视图，确认 15 个教程完整可选；逐项抽查复制结果不含掩码
