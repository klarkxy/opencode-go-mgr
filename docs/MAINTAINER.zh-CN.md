...
`pnpm run release:check` 只做版本和签名密钥预检，不构建原生安装包。生产 CI
会签名一个临时 payload，再用 `TAURI_UPDATER_PUBLIC_KEY` 验证；公私钥缺失或不
匹配时，会在平台矩阵占用 runner 时间之前失败。

## CI 工作流

`.github/workflows/quality.yml` 是可复用质量门：PR 和 `main` push 自动运行，
`release.yml` 发版时只调用一次。单台 Ubuntu runner 完成格式检查、锁定依赖的
Rust/Node 测试、TypeScript 检查、Vite 生产构建、Clippy、`DESIGN.md` lint 与
Compose 校验。兼容的运行共享 Node/pnpm 和 Rust 构建缓存；PR 只恢复 Rust 缓存，
不写回。

`.github/workflows/release.yml` 由 `workflow_dispatch` 和 `v*` tag 触发。手动候
选构建可选 Windows x64、macOS Universal、Linux x64 或全部平台；tag 始终强制
完整三平台矩阵。质量门与 Windows 预检并行：预检会解析抽出的安装器冒烟脚本、运
行发布辅助测试、校验所有版本清单，并证明生产升级公私钥匹配。

预检通过后，每个选中的原生 runner 恢复对应 Rust 缓存，安装依赖，只执行带签名的
`pnpm run build`、CLI/GUI 冒烟并上传保留 7 天的 `release-<platform>`。通用的
测试、类型和 lint 不再在三台原生 runner 上重复执行。
...
PR 会自动运行平台无关质量门，但原生安装包/打包冒烟仍只在手动候选或 tag 流程运
行。容器工作流只覆盖 `linux/amd64`，并且只在 Release 发布后或手动触发时运行。
...
5. 等待 `quality`、`preflight`、全部原生矩阵 job、`draft-release` 和
...
- [ ] 可复用质量门全绿；原生矩阵前的签名 `release:check` 通过；选中的每个
      `pnpm run build` 与平台冒烟全绿。
