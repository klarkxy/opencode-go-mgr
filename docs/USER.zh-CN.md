...
- **Windows GUI**：直接运行新版安装包覆盖安装。在 Windows **已安装的应用**
  中卸载；卸载程序会询问是否删除 `%USERPROFILE%\.ocg-mgr`。
...
- **Docker**：备份后更新代码并执行 `docker compose up -d --build`。
  `docker compose down` 只删容器、保留 `ocg-data`；`docker compose down -v`
  会永久删除卷，只能在确认备份有效且确实要重置时使用。
...
以和 Key 一起填入 OpenCode‑Go `username` 与 `password`；Key 与保存的密码在
磁盘上都只做混淆。必要时 Gateway 用密码刷新上游会话。
...
Gateway API 必须携带 **Gateway Key**，既可使用 `Authorization: Bearer <key>`，
也可使用兼容 Anthropic 的 `x-api-key: <key>`。管理面板的鉴权模式取决于监听
地址：
...
账号 Key 无关，Gateway 会从 SQLite 取出账号 Key，以自己的
`Authorization: Bearer <opencode-go-key>` 头转给上游。

三个协议的最小 POSIX shell 检查：
...
是上游账单视图。上游 usage chunk 会提供准确的流式 token 数量，但费用始终按
本地模型价格表估算。没有 usage chunk 时，日志记 `success_no_usage`。
...
- 流式 token 数量仅在上游发出 usage chunk 时准确；费用始终使用本地价格表。
