...
当前暂存版本会隐藏 **扩展 → CPA** 并强制关闭 CPA 路由；完成后续独立验证前请保持该 profile 关闭。验证门槛重新开放后，再填入与 `cpa-config.yaml` 相同的 CPA 推理 Key 和 Management password，运行应用级检测并在 CPA 内完成 OAuth。OCG 容器不会替你启动、停止、升级或 health-check CPA。
