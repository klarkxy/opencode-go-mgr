...
**账号** 的 **新增账号** 左侧上方为 **Plan**、下方为 **API**，右侧直接填写表单。**Custom API** 固定排在 API 第一项，搜索时也保留入口；普通 API、聚合服务和平台账号都归入 API。固定预设提供协议、鉴权、地址和官网有据可查的默认对话模型，填写对应服务的 **Key** 即可保存；名称和模型可在可选设置中调整。Azure、Bedrock 仍需填写资源或区域地址，以及模型／部署信息。保存时同时创建供应商和首个账号。要给已有供应商添加另一个账号，请选择该已保存的供应商。Zen
Free 是系统管理的单例，不会出现在新增列表里，只在账号列表中启停。选中 OpenCode Go
后，右侧直接填写已有 Key，并保留独立的 **注册新账号（Beta）** 流程：
