...
**新增账号** 先区分已有连接与新服务。选择已有供应商，会沿用其已保存的地址、协议和模型，只添加另一把 Key，也包括在 **供应商** 页保存、尚未填写 Key 的用户定义供应商；选择新服务，再浏览 Plan/API 预设或添加平台站点。保存预设会同时创建供应商和首个账号。填写 Key 前始终可查看连接摘要，地区和套餐变体使用紧凑选择器。Key 只在本页收集。

**已启用** 表示允许使用这份配置，不代表上游认证或模型测试已成功。测试结果保留在测试弹窗中。用户定义供应商没有建模订阅周期，包括从 Plan 预设创建的供应商：其账号不显示推算的购买日期、到期倒计时或到期提醒。既有存储中的购买日期锚点保留兼容，但不作为已经确认的计费事实展示。
...
给现有供应商追加 Key 时，选择已有连接，不会再创建另一份供应商配置。新服务入口包含 Plan/API 预设、Custom API 与平台类型。搜索匹配厂商、变体、预设名称和端点主机；选中搜索结果后，清空搜索仍保留准确变体。Zen Free 是后端管理的单例，只在账号列表启停；OpenCode Go 在宿主支持时仍提供可选的托管注册入口。
