...
**别名** 是独立的核心页面，因为它的只读表覆盖全部 Provider 合约、用户定义供应商映射与 Custom 账号，而不是当前选中的供应商。它把现有合约和账号能力汇总成公开名称，并展示可路由性与精确上游身份。它不会新建 Alias API、store、cache 或编辑器。Custom 映射只能在**账号**页编辑。
