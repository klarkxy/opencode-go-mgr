...
The rail lists Providers that already have at least one account, grouped by Plan/API. Built-in and preset Providers are treated the same: they stay off the rail until you add an account on **Accounts**. Saving a connection without a Key does not leave a row here; add the Key on **Accounts**, then the Provider appears. **Add Provider** in the rail footer opens the preset browser in the main pane, keeping available templates separate from configured Providers; existing preset bookmarks open the same embedded creation form. That form saves connection configuration only and does not collect a Key. Saved connections retain their preset brand where provenance is known, without certifying an edited address as official. The model catalog has its own model search and enabled-state filter; searching the Provider list does not search models. Mapping tables keep both public and upstream names accessible on narrow screens.
