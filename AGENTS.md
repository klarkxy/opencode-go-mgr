...
- Product: OCG Manager, a local multi-Plan operations console. Routable: OpenCode Go, Zen Free, Command Code GOAT, MiniMax CN Token Plan, Kimi Code CN, Ollama Cloud, Custom API, and user-defined Providers created on Providers; CPA is a separately routable static external integration when configured and enabled. Ollama Cloud is a sealed fixed-origin Plan (`https://ollama.com`), Chat Completions only, Bearer; its saved/raw catalog IDs do not join `/v1/models` or the Go Alias registry, and actual 429 uses generic cooldown/fallback. Command Code uses one Provider-level model/protocol semantic: GOAT preset rows default on, additional discovered rows default off, the public `/models` catalog is not Key verification, and there is no account GOAT/All or Max mode.
...
- Access credentials are SQLite `access_keys` rows (current schema v37); plaintext leaves the process only via the session-protected `GET /dashboard/api/v3/connection`. Persistence details: `docs/maintainer/storage-migration.md`.
...
- `crates/ocg-core/src/db.rs`: SQLite schema, migrations, queries; `CURRENT_SCHEMA_VERSION = 37`.
...
- `auto_start` is only available in Windows x64, macOS, and Linux x64 release/installed Tauri desktop processes; the HTTP dashboard shows the toggle based on runtime capability; dev builds, CLI, and Docker do not expose this setting.
