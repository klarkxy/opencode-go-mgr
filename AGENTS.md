...
- Dashboard navigation has seven fixed core views in this order: Dashboard / Access Keys / Accounts / Providers / Applications / Logs / Settings. A divider below Settings starts the optional **Extensions** group for non-core product surfaces; CPA is its first local-only entry. `browser` is a managed-session overlay page. Access credentials are shown as **Key** (never “Gateway Key”); the design system is governed by `DESIGN.md` + `src/theme.ts`.
