...
- `.github/workflows/quality.yml` 在 PR / `main` 上复用 Linux 主质量门和 Windows Tauri 定向测试；`release.yml` 的手动候选（即使选择 tag ref）始终无签名且可只构建指定平台，只有 `v*` tag 的 push 事件才构建三平台，并在发布前校验恰好 15 个附件、升级签名与 GitHub 服务端 digest。公开发布默认关闭，只有 `OCG_RELEASE_APPROVAL_ENABLED=true` 且 `release` Environment 审批通过才会把已验证 draft 转为 published。
