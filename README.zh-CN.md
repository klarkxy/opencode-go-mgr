...
- **签名桌面升级**：设置页可检查 GitHub 最新 Release。内置升级公钥的已安装
  桌面版可下载、验签并原位安装；开发构建、CLI 与 Docker 仍走发布页/手动替换。
...
件同级，`serve` 才能提供管理面板。`SHA256SUMS`、签名升级行为、SmartScreen/
Gatekeeper 提示，以及不支持清单（ARM64、32 位 x86、RPM、Snap、应用商店）
见 [维护者指南](docs/MAINTAINER.zh-CN.md)。
